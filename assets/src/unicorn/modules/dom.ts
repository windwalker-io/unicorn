...
import { UnicornPlugin } from '@/unicorn/types';
import { defData } from '@/unicorn/utilities';
...
export function singletonModule<T = any, E extends Element = Element>(
  ele: string,
  name: string,
  callback?: ((el: E) => any)
): (T | null)[];
...
export function uri(type: string) {
  return data('unicorn.uri')[type];
}

export function asset(type: string) {
