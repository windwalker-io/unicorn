...
//#region ../../../../node_modules/flatpickr/dist/plugins/monthSelect/index.js
var require_monthSelect = /* @__PURE__ */ __commonJSMin(((exports, module) => {
			...
			return function(fp) {
				...
				function buildMonths() {
					...
					for (var i = 0; i < 12; i++) {
						...
						if (month.dateObj.getMonth() === (/* @__PURE__ */ new Date()).getMonth() && month.dateObj.getFullYear() === (/* @__PURE__ */ new Date()).getFullYear()) month.classList.add("today");
						month.textContent = monthToStr(i, config.shorthand, fp.l10n);
						month.addEventListener("click", selectMonth);
						frag.appendChild(month);
					}
					self.monthsContainer.appendChild(frag);
					if (fp.config.minDate && fp.currentYear === fp.config.minDate.getFullYear()) fp.prevMonthNav.classList.add("flatpickr-disabled");
					else fp.prevMonthNav.classList.remove("flatpickr-disabled");
					if (fp.config.maxDate && fp.currentYear === fp.config.maxDate.getFullYear()) fp.nextMonthNav.classList.add("flatpickr-disabled");
				...
				function selectYear() {
					...
					if (fp.rContainer) fp.rContainer.querySelectorAll(".flatpickr-monthSelect-month").forEach(function(month) {
						month.dateObj.setFullYear(fp.currentYear);
						if (fp.config.minDate && month.dateObj < fp.config.minDate || fp.config.maxDate && month.dateObj > fp.config.maxDate) month.classList.add("flatpickr-disabled");
				...
				function setMonth(date) {
					...
					switch (fp.config.mode) {
						...
						case "multiple":
							...
							break;
						case "range":
							if (fp.selectedDates.length === 2) selectedDates = [selectedDate];
							else {
...
//#endregion
export default /* @__PURE__ */ require_monthSelect();
